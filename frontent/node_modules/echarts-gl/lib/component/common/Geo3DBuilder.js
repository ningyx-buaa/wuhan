...
var lines3DGLSL = require("../../util/shader/lines3D.glsl.js");
...
function Geo3DBuilder(api) {
  ...
  this._currentMap = ''; // Cache triangulation result

  this._triangulationResults = {}; // this._triangulator = new Triangulation();
  ...
  this._shadersMap = graphicGL.COMMON_SHADERS.reduce(function (obj, shaderName) {
    ...
    obj[shaderName].define('fragment', 'DOUBLE_SIDED'); // obj[shaderName].define('both', 'WIREFRAME_TRIANGLE');
  ...
  this._labelsBuilder = new LabelsBuilder(1024, 1024, api); // Give a large render order.
...
Geo3DBuilder.prototype = {
  ...
  update: function (componentModel, geo3D, ecModel, api) {
    ...
    if (geo3D.map !== this._currentMap || enableInstancing && !this._polygonMesh || !enableInstancing && !this._polygonMeshesMap) {
      this._currentMap = geo3D.map; // Reset meshes
    ...
    this._updateDebugWireframe(componentModel, geo3D); // Reset some state.
  ...
  _prepareInstancingMesh: function (componentModel, geo3D, shader, api) {
    ...
    var polygonTriangleCount = 0; // TODO Lines
  ...
  _updateRegionMesh: function (componentModel, geo3D, shader, api, instancing) {
    ...
    geo3D.regions.forEach(function (region) {
      ...
      var opacity = retrieve.firstNotNull(itemStyleModel.get('opacity'), 1.0); // Use visual color if it is encoded by visualMap component
      ...
      if (instancing) {
        var newOffsets = this._updatePolygonGeometry(componentModel, polygonMesh.geometry, region, regionHeight, vertexOffset, triangleOffset, color);
        ...
        this._updatePolygonGeometry(componentModel, polygonMesh.geometry, region, regionHeight);
      } // Update lines.
      ...
      if (!instancing) {
        ...
        this._moveRegionToCenter(polygonMesh, linesMesh, hasLine); // Bind events.
        ...
        polygonMesh.on('mouseout', this._onmouseout, this); // Update tangents
    ...
    if (instancing) {
      ...
      polygonMesh.geometry.updateBoundingBox(); // Update tangents
  ...
  _onmouseover: function (e) {
    if (e.target) {
      var dataIndex = e.target.eventData ? this._data.indexOfName(e.target.eventData.name) : e.target.dataIndex;
  ...
  _onmouseout: function (e) {
    if (e.target) {
      ...
      if (this._polygonMesh) {
        ...
        if (dataIndex != null) {
          this.downplay(dataIndex); // TODO Merge with onmouseover
  ...
  _triangulation: function (geo3D) {
    this._triangulationResults = {}; // var triangulator = this._triangulator;
  ...
  _updatePolygonGeometry: function (componentModel, geometry, region, regionHeight, vertexOffset, triangleOffset, color) {
    // FIXME
    ...
    if (!instancing) {
      ...
      indices = geometry.indices = geoInfo.vertexCount > 0xffff ? new Uint32Array(geoInfo.triangleCount * 3) : new Uint16Array(geoInfo.triangleCount * 3);
    }
  ...
  highlight: function (dataIndex) {
    ...
    var emphasisOpacity = retrieve.firstNotNull(emphasisItemStyleModel.get('opacity'), data.getItemVisual(dataIndex, 'opacity'), 1);

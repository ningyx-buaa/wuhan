...
Grid3DAxis.prototype.update = function (grid3DModel, labelIntervalFuncs, axisLabelSurface, api) {
  ...
  if (axisTickModel.get('show')) {
    ...
    var ticksCoords = axis.getTicksCoords(); // TODO Automatic interval
  ...
  if (axisLabelModel.get('show')) {
    ...
    var categoryData = axisModel.get('data'); // TODO Automatic interval
    ...
    for (var i = 0; i < labelsCoords.length; i++) {
      ...
      if (categoryData && categoryData[ticks[i]] && categoryData[ticks[i]].textStyle) {
        itemTextStyleModel = new echarts.Model(categoryData[ticks[i]].textStyle, axisLabelModel, axisModel.ecModel);
      }
      ...
      echarts.graphic.setTextStyle(textEl.style, itemTextStyleModel, {
        ...
        axis.type === 'category' ? labels[i] : axis.type === 'value' ? ticks[i] + '' : ticks[i], i) : textColor,

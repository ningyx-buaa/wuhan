...
function resizeMapbox(mapboxModel, api) {
  ...
  this.boxHeight = mapboxModel.get('boxHeight'); // this.updateTransform();
}
...
var mapboxCreator = {
  ...
  create: function (ecModel, api) {
    ...
    ecModel.eachComponent('mapbox', function (mapboxModel) {
      ...
      mapboxCoordSys.viewGL = mapboxModel.__viewGL; // Inject resize
    ...
    ecModel.eachSeries(function (seriesModel) {
      if (seriesModel.get('coordinateSystem') === 'mapbox') {
        ...
        if (!mapboxModel) {
          throw new Error('mapbox "' + retrieve.firstNotNull(seriesModel.get('mapboxIndex'), seriesModel.get('mapboxId'), 0) + '" not found');
        }

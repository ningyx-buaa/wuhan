...
function checkAndDispose(renderer, resourceMap, maxCount) {
  var count = 0; // FIXME not allocate array.
    ...
    if (dataIndex != null) {
      if (dataIndex !== this._lastDataIndex) {
        if (parseInt(this._lastDataIndex, 10) >= 0) {
          ...
          eventProxy.seriesIndex = this._lastSeriesIndex; // FIXME May cause double events.
      ...
      if (eventData !== this._lastEventData) {
        if (this._lastEventData != null) {
          eventProxy.eventData = this._lastEventData; // FIXME May cause double events.
